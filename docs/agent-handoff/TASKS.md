...
| CASH-SAVINGS-BALANCES | Enter balances for the plain "Cash" account and "Personal Savings" account — Simplifi's Savings group has only ever been seen collapsed to one $91,770.85 total, never broken into individual accounts | _(unclaimed)_ | _(none yet)_ | Live data entry via `/api/financial/account-balances`, no code | needs an expanded-Savings-group screenshot from Jason | Both accounts show a real balance instead of an empty entry row |
