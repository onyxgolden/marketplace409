...
| Recurring autopay | Consent/control foundation | Tenant-controlled enrollment and cancellation preserve consent, timing, reminder, retry, and status evidence. Automatic debit remains inactive until Stripe returns a reusable payment method and mandate; provider setup, execution, and retry processing remain. |
...
| Accounting and bank reconciliation | Payment/reversal posting implemented | Succeeded payments post idempotently to rental income; refunds and disputes recompute balances and post separate negative adjustments while preserving receipts. Processor fees, balance transactions, payout matching, and automated Stripe refund ingestion remain. |
