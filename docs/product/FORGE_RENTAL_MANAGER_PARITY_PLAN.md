...
| Lease and rent schedule | Lifecycle controls implemented | Draft lease, activation, monthly schedule, effective-date guards, idempotent charge generation, auditable renewals/amendments, dated replacement schedules, and separately labeled proration charges exist. Current FORGE records terms and stores uploaded lease documents; it does not generate or reproduce a Texas REALTORS® form. Editable lease preparation must use either a licensed approved-forms integration or a separately attorney-reviewed FORGE template. |
