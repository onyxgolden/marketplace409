...
| Lease and rent schedule | Working sandbox slice | Draft lease, activation, monthly schedule, effective-date guards, and idempotent charge generation exist. Current FORGE records terms and stores uploaded lease documents; it does not generate or reproduce a Texas REALTORS® form. Editable lease preparation must use either a licensed approved-forms integration or a separately attorney-reviewed FORGE template. Renewal, amendments, prorating, and late-fee rules remain. |
