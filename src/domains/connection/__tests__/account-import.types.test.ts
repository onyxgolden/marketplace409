...
describe("AccountImportResult", () => {
  it("marks persisted connection accounts as ready for transaction import", () => {
    ...
    const result = toAccountImportResult(input, {
      provider: "plaid",
      connectionId: "connection-1",
    ...
    expect(result.importedAccountCount).toBe(3);
    expect(result.skippedAccountCount).toBe(1);
    ...
    expect(result.importedAt).toBe("2026-07-02T03:00:00.000Z");
