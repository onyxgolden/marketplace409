...
function makeProvider(
  ...
  const providerName = overrides.provider ?? "plaid";

  return {
    provider: providerName,
    displayName: overrides.displayName ?? "Plaid",

    capabilities: overrides.capabilities ?? (() => ({
      connectionId: `${providerName}_capabilities`,
      capabilities: ["import_accounts", "manual_sync"],
    ...
    connect: overrides.connect ?? (async () => makeInput(providerName).connection),
    ...
    importData: overrides.importData ?? (async (connection) => ({
      provider: providerName,
...
describe("AccountImportService", () => {
  it("imports accounts through the registered provider", async () => {
    ...
    const service = new AccountImportService(registry);

    const result = await service.importAccounts(makeInput());

    expect(result.provider).toBe("plaid");
    expect(result.connectionId).toBe("connection-1");
    expect(result.success).toBe(true);
    expect(result.importedAccountCount).toBe(2);
    expect(result.skippedAccountCount).toBe(0);
    expect(result.failedAccountCount).toBe(0);
    expect(result.readyForTransactionImport).toBe(true);
  });
  ...
  it("rejects missing providers", async () => {
    ...
    await expect(service.importAccounts(makeInput()))
      .rejects
      .toThrow("No connection provider registered for account import.");
  });

  it("rejects providers without account import capability", async () => {
    const provider = makeProvider({
      capabilities: () => ({
        connectionId: "plaid_capabilities",
    ...
    await expect(service.importAccounts(makeInput()))
      .rejects
      .toThrow("Connection provider does not support account import.");
  });
});
