...
import { ConnectionPersistenceService } from "../connection-persistence.service";
...
import type { ConnectionProvisioningResult } from "../connection-provisioning.types";

const buildProvisioningResult = (
  overrides: Partial<ConnectionProvisioningResult> = {},
...
describe("ConnectionPersistenceService", () => {
  it("persists a provisioned connection through repository contracts", () => {
    ...
    const service = new ConnectionPersistenceService(
      connectionRepository,
      credentialReferenceRepository,
      institutionReferenceRepository,
    );

    const provisionedConnection = buildProvisioningResult();

    const result = service.persist(provisionedConnection);

    expect(
      connectionRepository.getById("connection-1"),
    ).toEqual(provisionedConnection.connection);

    expect(
      credentialReferenceRepository.getById("credential-1"),
    ).toEqual(provisionedConnection.credentialReference);

    expect(
      institutionReferenceRepository.getById("institution-1"),
    ).toEqual(provisionedConnection.institutionReference);

    expect(result.connection).toEqual(provisionedConnection.connection);
    expect(result.credentialReference).toEqual(
      provisionedConnection.credentialReference,
    );
    expect(result.institutionReference).toEqual(
      provisionedConnection.institutionReference,
    );
    expect(result.provisionedAt).toBe(
      provisionedConnection.provisionedAt,
    );
    expect(result.readyForImport).toBe(true);
    expect(result.persistedAt).toEqual(expect.any(String));
  });
  ...
  it("does not require provider-specific repository implementations", () => {
    ...
    const result = service.persist(
      buildProvisioningResult({
        connection: {
          id: "connection-2",
          provider: "manual",
          externalConnectionId: "manual-source-1",
          status: "connected",
          credentialReferenceId: "credential-2",
          createdAt: "2026-07-02T00:00:00.000Z",
          updatedAt: "2026-07-02T00:00:00.000Z",
        },
        credentialReference: {
          id: "credential-2",
          provider: "manual",
          externalCredentialId: "manual-source-1",
          status: "active",
          createdAt: "2026-07-02T00:00:00.000Z",
          updatedAt: "2026-07-02T00:00:00.000Z",
        },
        institutionReference: {
          ...
          provider: "manual",
          externalInstitutionId: "manual-institution-1",
          name: "Manual Source",
          type: "other",
          createdAt: "2026-07-02T00:00:00.000Z",
          updatedAt: "2026-07-02T00:00:00.000Z",
    ...
    expect(result.connection.provider).toBe("manual");
    expect(result.readyForImport).toBe(true);
  });
});
