...
describe("Rental Manager route", () => {
  ...
  it("echoes the authenticated acting user and canonical (effective) owner id", async () => {
    ...
    const tables = { rent_charges: empty, rental_units: empty, rental_tenants: empty, rent_schedules: empty,
      ...
      rental_maintenance_work_events: empty, rental_lease_preparations: empty, rental_lease_preparation_versions: empty, rental_lease_signatures: empty,
  ...
  it("returns financial events beyond PostgREST's 1000-row default page cap, paginating through every page", async () => {
    ...
    const tables = { rent_charges: result([]), rental_units: result([]), rental_tenants: result([]), rent_schedules: result([]),
      ...
      rental_lease_preparation_versions: result([]), rental_lease_signatures: result([]), rental_autopay_enrollments: result([]), renters_insurance_policies: result([]),
  ...
  it("dashboard collectionSummary distinguishes FORGE-collectible from externally-managed open charges", async () => {
    ...
    const tables = { rent_charges: result(charges), rental_units: result([]), rental_tenants: result([]), rent_schedules: result(schedules),
      ...
      rental_maintenance_work_events: result([]), rental_lease_preparations: result([]), rental_lease_preparation_versions: result([]), rental_lease_signatures: result([]),
