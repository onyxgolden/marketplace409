...
const tables={
  ...
  financial_events:[{id:"e1",event_date:"2026-01-01",description:"STARLINK",amount:120,transaction_kind:"expense",normalized_category:"utilities",property_id:"kent",business_scope:"business"},{id:"e2",event_date:"2026-01-02",description:"GROCERIES",amount:80,transaction_kind:"expense",normalized_category:"groceries",property_id:null,business_scope:"personal"}],
...
it("exports the account ledger as CSV",async()=>{const response=await GET(new Request("https://example.test/api/rental/reports?report=account-ledger&format=csv"));expect(response.headers.get("content-type")).toContain("text/csv");expect(response.headers.get("content-disposition")).toContain("forge-account-ledger");const csv=await response.text();expect(csv).toContain('"STARLINK"');expect(csv).not.toContain("GROCERIES");});
it("excludes personal-scope transactions from the account ledger and income & expense statement",async()=>{const ledger=await GET(new Request("https://example.test/api/rental/reports?report=account-ledger"));expect((await ledger.json()).report.summary).toMatchObject({transactionCount:1,totalDebits:120});const statement=await GET(new Request("https://example.test/api/rental/reports?report=income-expense-statement"));expect((await statement.json()).report.summary).toMatchObject({expenses:120});});
...
it("returns and exports the standalone business expense report",async()=>{const response=await GET(new Request("https://example.test/api/rental/reports?report=business-expenses&scope=property&propertyId=kent&category=utilities"));const body=await response.json();expect(response.status).toBe(200);expect(body.report.summary).toEqual({transactionCount:1,totalExpenses:120});expect(body.report.rows[0]).toMatchObject({description:"STARLINK",propertyId:"kent"});const csv=await GET(new Request("https://example.test/api/rental/reports?report=business-expenses&format=csv"));expect(csv.headers.get("content-disposition")).toContain("forge-business-expenses");expect(await csv.text()).toContain('"STARLINK"');});
