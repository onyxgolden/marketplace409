// Call Shield API request guard.
//
// The Android companion shell is a non-browser client: it sends the user's
// Supabase access token as `Authorization: Bearer <token>`. The shared
// createAuthenticatedForgeApplication helper is cookie-based (@supabase/ssr)
// and cannot see that header, so this guard handles the Bearer <redacted> first
// and falls back to the cookie path for browser/WebView callers.
//
...
export async function guardCallShieldRequest(request) {
  ...
  if (token) {
    ...
    // with the user's own JWT as its Authorization header so every
    // data-plane call runs as that user and RLS (owner_id =
    // auth.uid()::text) applies.
