...
// owner UI, matching how an inbox naturally works. mark_pf_conversation_read_by_owner is a no-op
// (updates zero rows) when no conversation exists yet for this borrower, so this is safe to call
// even before any message has ever been sent.
export async function GET(request, { params }) {
  const authenticated = await createAuthenticatedPrivateFinancingApplication();
  if (authenticated.response) return authenticated.response;
  const { borrowerId } = await params;
  const conversation = await authenticated.supabaseClient.from("private_financing_conversations")
    .select("id").eq("borrower_id", borrowerId).maybeSingle();
  ...
  const { data, error } = await authenticated.supabaseClient.from("private_financing_conversation_messages")
  ...
  const { error: readError } = await authenticated.supabaseClient.rpc("mark_pf_conversation_read_by_owner", { p_borrower_id: borrowerId });
...
export async function POST(request, { params }) {
  ...
  const { data, error } = await authenticated.supabaseClient.rpc("send_pf_conversation_owner_message", {
    p_borrower_id: borrowerId, p_body: body.body.trim(),
  });
