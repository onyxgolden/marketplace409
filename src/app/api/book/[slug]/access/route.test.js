...
describe("public reservation access route", () => {
  ...
  it("returns only the RPC-shaped release state", async () => {
    const rpc = vi.fn().mockResolvedValue({ data: { publicName: "Pine Cabin", available: false, availableAt: "2026-10-01T15:00:00Z", arrivalInstructions: null }, error: null });
    ...
    expect((await response.json()).access.arrivalInstructions).toBeNull();
