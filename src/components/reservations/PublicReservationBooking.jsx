...
export default function PublicReservationBooking({ slug }) {
  const [listing, setListing] = useState(null), [form, setForm] = useState(initial), [preview, setPreview] = useState(null), [token, setToken] = useState("");
  ...
  useEffect(() => { const controller = new AbortController(); fetch(`/api/book/${encodeURIComponent(slug)}`, { signal: controller.signal }).then(async response => { const body = await response.json(); if (!response.ok) throw new Error(body.error); setListing(body.listing); setState({ loading: false, busy: false, error: "" }); }).catch(error => { if (error.name !== "AbortError") setState({ loading: false, busy: false, error: error.message }); }); return () => controller.abort(); }, [slug]);
  ...
  if (state.loading) return <main className="mx-auto max-w-3xl p-6"><p role="status">Loading stay…</p></main>;
  if (state.error && !listing) return <main className="mx-auto max-w-3xl p-6"><p role="alert">{state.error}</p></main>;
  if (confirmation) return <main className="mx-auto max-w-3xl p-6"><section className="rounded-3xl border bg-white p-7 shadow-sm dark:border-slate-800 dark:bg-slate-900"><p className="text-xs font-black uppercase tracking-widest text-emerald-700">Reservation confirmed</p><h1 className="mt-2 text-3xl font-black">{confirmation.publicName}</h1><p className="mt-3">Confirmation <b>{confirmation.id}</b></p><p>{confirmation.checkIn} through {confirmation.checkOut}</p><p className="mt-3 text-xl font-black">Total due: {money(confirmation.totalDueCents, confirmation.currencyCode)}</p><p className="mt-2 rounded-xl bg-amber-50 p-3 font-bold text-amber-950">No payment was collected during this booking.</p>{confirmation.arrivalInstructions && <p className="mt-3"><b>Arrival instructions:</b> {confirmation.arrivalInstructions}</p>}<p className="mt-3 text-sm text-slate-600 dark:text-slate-300">A confirmation email has been queued for delivery.</p></section></main>;
  const q = preview?.quote;
  return <main className="mx-auto max-w-3xl p-6 text-slate-950 dark:text-slate-100"><header><p className="text-xs font-black uppercase tracking-widest text-sky-700 dark:text-sky-400">409 Marketplace stays</p><h1 className="mt-2 text-3xl font-black">{listing.publicName}</h1><p className="mt-2 text-slate-600 dark:text-slate-300">{listing.publicDescription || "Book this RV or cabin stay directly."}</p><p className="mt-2 text-sm capitalize">{label(listing.inventoryType)} · Up to {listing.maximumGuests} guests · {listing.minimumNights} night minimum</p></header>
    {state.error && <p role="alert" className="mt-5 rounded-xl border border-rose-300 bg-rose-50 p-3 font-bold text-rose-900">{state.error}</p>}
