...
/**
 * ProposalPrintOverlay — the single FORGE proposal print flow (slice 4).
 *
 * Mirrors the PrintSheetOverlay pattern: a full-screen preview of the
 * ProposalPrintView document at exact Letter-portrait physical dimensions,
...
import ProposalPrintView, { PROPOSAL_PAGE_CSS } from "./ProposalPrintView";
...
export default function ProposalPrintOverlay({ estimate, onClose }) {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      @media print {
        ...
        .forge-print-portal .forge-proposal-sheet {
  ...
  if (!estimate || typeof document === "undefined") return null;
  ...
  return createPortal(
    ...
    >
      ...
      >
        <span style={{ fontWeight: 700, fontSize: 14 }}>Print proposal</span>
        <button
        ...
        <span style={{ fontSize: 12, color: "#9ca3af", flexBasis: "100%" }}>
          Prints the live on-screen estimate — the numbers below are what prints.
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div style={{ zoom, boxShadow: "0 8px 40px rgba(0,0,0,0.6)" }}>
          <ProposalPrintView estimate={estimate} />
        </div>
      </div>
    </div>,
    document.body,
