...
export default function RentecOwnerInputResolution({ resolution, busy = false, onResolve }) {
  ...
  if (!requirements.length) return <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4">
    ...
    <p className="mt-2 text-sm text-emerald-800">The preview checksum now covers the validated inputs. No records were saved or imported.</p>
  </div>;
  ...
  function submit(event) {
    ...
    for (const requirement of requirements) {
      if (requirement.type === "tenant_email") tenantEmails[requirement.sourceId] = values[`tenant_email:${requirement.sourceId}`] || "";
      if (requirement.type === "rent_due_day") leaseRentDueDays[requirement.sourceId] = Number(values[`rent_due_day:${requirement.sourceId}`] || 0);
    }
    onResolve?.({ tenantEmails, leaseRentDueDays });
  }
  ...
  return <form onSubmit={submit} className="rounded-xl border border-amber-300 bg-amber-50 p-4">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      {requirements.map((requirement) => {
        ...
        return <label key={id} className="rounded-lg border bg-white p-3 text-sm font-bold">
    ...
    <p className="mt-3 text-sm font-bold text-amber-900">Preview only: this does not import, activate billing, or save the entered values.</p>
  </form>;
}
