...
export default function RentalDocumentsPanel(){
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-bold uppercase tracking-widest text-amber-700">Document library</p>
    ...
    <div className="mt-8 border-t pt-6"><h3 className="font-black">Saved documents</h3>{documents.length===0?<p className="mt-2 text-sm text-slate-500">No rental documents uploaded.</p>:documents.map(document=><div key={document.id} className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><strong>{document.title}</strong><p className="text-sm capitalize text-slate-500">{document.category} · {document.tenant_visible?"Visible to tenant":"Private to landlord"}</p></div><a href={document.download_url} target="_blank" rel="noreferrer" className="font-bold text-blue-700 underline">Open document</a></div>)}</div>
  </section>;
}
