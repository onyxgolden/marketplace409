"use client";
import { useEffect, useMemo, useState } from "react";
import { Elements } from "@stripe/react-stripe-js";
import { loadStripe } from "@stripe/stripe-js";
import TenantAutopayBankSetupForm from "./TenantAutopayBankSetupForm";

const STRIPE_PUBLISHABLE_KEY = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
// Stashed before bank confirmation so activation can resume if the bank's own login flow
// takes the tenant away from the portal and Stripe redirects back afterwards.
const SETUP_STORAGE_KEY = "forge-autopay-bank-setup";

function isValidPublishableKey(key) {
  return typeof key === "string" && /^pk_(test|live)_/.test(key);
}

async function postOperation(payload) {
  const response = await fetch("/api/rental/portal", { method: "POST",
    headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  const body = await response.json();
  if (!response.ok) throw new Error(body.error || "Request failed.");
  return body;
}

export default function TenantAutopayPanel({ rentals, onChanged }) {
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  // Bank-link in progress: { enrollmentId, setupIntentId, clientSecret, connectedAccountId }
  const [setup, setSetup] = useState(null);
  const [stripeLoadFailed, setStripeLoadFailed] = useState(false);
  const current = (rentals || []).flatMap((r) => r.autopayEnrollments || [])
    .find((e) => ["setup_required", "active", "paused"].includes(e.status));
  const needsBankLink = current?.status === "setup_required" && current?.paymentMethodType === "us_bank_account";

  const stripePromise = useMemo(() => {
    if (!setup || !isValidPublishableKey(STRIPE_PUBLISHABLE_KEY)) return null;
    return loadStripe(STRIPE_PUBLISHABLE_KEY, { stripeAccount: setup.connectedAccountId });
  }, [setup]);

  useEffect(() => {
    if (!stripePromise) return;
    let cancelled = false;
    stripePromise.then((stripe) => { if (!cancelled && !stripe) setStripeLoadFailed(true); })
      .catch(() => { if (!cancelled) setStripeLoadFailed(true); });
    return () => { cancelled = true; };
  }, [stripePromise]);

  // Resume after a bank OAuth redirect: Stripe returns to ?autopay=setup-returned and the
  // pending setup intent was stashed in sessionStorage before confirmation.
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (new URLSearchParams(window.location.search).get("autopay") !== "setup-returned") return;
    let pending = null;
    try { pending = JSON.parse(window.sessionStorage.getItem(SETUP_STORAGE_KEY)); } catch { pending = null; }
    if (!pending?.enrollmentId || !pending?.setupIntentId) return;
    window.sessionStorage.removeItem(SETUP_STORAGE_KEY);
    const url = new URL(window.location.href);
    url.searchParams.delete("autopay");
    window.history.replaceState(null, "", url.toString());
    (async () => {
      setBusy(true); setMessage("");
      try {
        await postOperation({ operation: "complete-autopay-setup",
          enrollmentId: pending.enrollmentId, setupIntentId: pending.setupIntentId });
        setMessage("Bank account linked. Automatic payments are now active.");
        await onChanged();
      } catch (error) { setMessage(error.message); } finally { setBusy(false); }
    })();
  }, [onChanged]);

  async function submit(payload) {
    setBusy(true); setMessage("");
    try {
      await postOperation(payload);
      setMessage(payload.operation === "cancel-autopay"
        ? "Autopay enrollment cancelled."
        : "Consent recorded. Link your bank account below to activate automatic payments.");
      await onChanged();
    } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  }

  async function startBankLink() {
    setBusy(true); setMessage(""); setStripeLoadFailed(false);
    try {
      const result = await postOperation({ operation: "create-autopay-setup", enrollmentId: current.id });
      const pending = { enrollmentId: result.enrollmentId, setupIntentId: result.setupIntentId,
        clientSecret: result.clientSecret, connectedAccountId: result.connectedAccountId };
      window.sessionStorage.setItem(SETUP_STORAGE_KEY,
        JSON.stringify({ enrollmentId: pending.enrollmentId, setupIntentId: pending.setupIntentId }));
      setSetup(pending);
    } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  }

  async function handleSetupSuccess(setupIntentId) {
    setBusy(true); setMessage("");
    try {
      await postOperation({ operation: "complete-autopay-setup",
        enrollmentId: setup.enrollmentId, setupIntentId });
      window.sessionStorage.removeItem(SETUP_STORAGE_KEY);
      setSetup(null);
      setMessage("Bank account linked. Automatic payments are now active.");
      await onChanged();
    } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  }

  function cancelSetup() {
    window.sessionStorage.removeItem(SETUP_STORAGE_KEY);
    setSetup(null);
  }

  return <section className="rounded-2xl border bg-white p-6 shadow-sm">
    <p className="text-sm font-bold uppercase tracking-widest text-amber-700">Automatic payments</p>
    <h2 className="mt-2 text-xl font-black">Autopay controls</h2>
    {setup ? (
      stripeLoadFailed ? <div role="alert" className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-800">
        <p className="font-bold">Unable to load the secure bank form</p>
        <p className="mt-1">Stripe could not be initialized. Nothing has been linked and your enrollment is unchanged.</p>
        <button type="button" onClick={cancelSetup} className="mt-3 rounded-xl border px-4 py-2 font-bold text-slate-700">Back</button>
      </div> : <div className="mt-4">
        <Elements stripe={stripePromise} options={{ clientSecret: setup.clientSecret, appearance: { theme: "stripe" } }}>
          <TenantAutopayBankSetupForm busy={busy} onSuccess={handleSetupSuccess} onCancel={cancelSetup} />
        </Elements>
      </div>
    ) : current ? <>
      <p className="mt-3 text-sm">Status: <strong>{current.status.replaceAll("_", " ")}</strong> · charge day {current.chargeDay}</p>
      {needsBankLink ? <>
        <p className="mt-2 text-sm text-slate-600">
          Consent is recorded. Link your bank account to activate automatic debits — nothing is charged until the account is linked.
        </p>
        <div className="mt-4 flex gap-3">
          <button disabled={busy} onClick={startBankLink}
            className="rounded-xl bg-slate-950 px-5 py-3 font-bold text-white disabled:opacity-50">
            {busy ? "Opening…" : "Link bank account"}
          </button>
          <button disabled={busy} onClick={() => submit({ operation: "cancel-autopay", enrollmentId: current.id, reason: "Cancelled by tenant in portal" })}
            className="rounded-xl border px-4 py-2 font-bold">Cancel autopay</button>
        </div>
      </> : <>
        <p className="mt-2 text-sm text-slate-600">
          {current.status === "setup_required"
            ? "No automatic debit can occur yet. Stripe payment-method and mandate setup is still required."
            : "You may cancel future automatic payments at any time."}
        </p>
        <button disabled={busy} onClick={() => submit({ operation: "cancel-autopay", enrollmentId: current.id, reason: "Cancelled by tenant in portal" })}
          className="mt-4 rounded-xl border px-4 py-2 font-bold">Cancel autopay</button>
      </>}
    </> : <form className="mt-4 grid gap-4" onSubmit={(event) => {
        event.preventDefault();
        const f = new FormData(event.currentTarget);
        submit({ operation: "request-autopay", leaseId: f.get("leaseId"), paymentMethodType: f.get("paymentMethodType"),
          chargeDay: Number(f.get("chargeDay")), reminderDaysBefore: Number(f.get("reminderDaysBefore")),
          consentConfirmed: f.get("consentConfirmed") === "on" });
      }}>
      <label className="text-sm font-bold">Lease
        <select name="leaseId" required className="mt-1 w-full rounded-xl border p-3 font-normal">
          {(rentals || []).map((r) => <option key={r.lease.id} value={r.lease.id}>{r.unit?.label || "Rental home"}</option>)}
        </select>
      </label>
      <label className="text-sm font-bold">Payment method
        <select name="paymentMethodType" className="mt-1 w-full rounded-xl border p-3 font-normal">
          <option value="us_bank_account">US bank account</option>
          <option value="card">Card</option>
        </select>
      </label>
      <div className="grid grid-cols-2 gap-3">
        <label className="text-sm font-bold">Charge day
          <input name="chargeDay" type="number" min="1" max="28" defaultValue="1" className="mt-1 w-full rounded-xl border p-3 font-normal" />
        </label>
        <label className="text-sm font-bold">Reminder days before
          <input name="reminderDaysBefore" type="number" min="0" max="14" defaultValue="3" className="mt-1 w-full rounded-xl border p-3 font-normal" />
        </label>
      </div>
      <label className="flex gap-3 rounded-xl bg-amber-50 p-4 text-sm">
        <input name="consentConfirmed" type="checkbox" required />
        <span>I authorize recurring rent payments under the schedule shown here. I understand enrollment is not active until Stripe securely verifies my payment method and mandate, and I may cancel future payments.</span>
      </label>
      <button disabled={busy} className="rounded-xl bg-slate-950 px-5 py-3 font-bold text-white disabled:opacity-50">
        {busy ? "Saving…" : "Continue autopay setup"}
      </button>
    </form>}
    {message ? <p role="status" className="mt-4 rounded-xl bg-blue-50 p-3 text-sm text-blue-900">{message}</p> : null}
  </section>;
}
