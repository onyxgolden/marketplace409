...
export default function RentalTenantPanel({ initialTenants = [], onNavigate: navigate }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-rental-tenant-setup>
    ...
    {tenants.length > 0 && <RentalRecordBrowser title="Tenants" records={tenants} selectedId={selectedId} onSelect={setSelectedId} getTitle={(tenant) => tenant.display_name} getSubtitle={(tenant) => { const propertyLabel = propertyLabelForTenant(tenant, leases, leaseMemberships, units); return <>{tenant.email} · {tenant.status || "Status not set"} · {propertyLabel || <span className="font-bold text-red-600">No active lease</span>}</>; }} getThumbnail={(tenant) => tenant.photo_url}>
