...
export default function ReservationsPanel() {
  ...
  return <section aria-label="Reservations" className="space-y-5">
    ...
    <form onSubmit={event => { event.preventDefault(); submit(editing ? "preview_modification" : "preview"); }} className="space-y-3 rounded-2xl border bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <Field label="Space"><select required value={form.unitId} onChange={event => change("unitId", event.target.value)} className="input"><option value="">Select a space</option>{inventory.map(item => <option key={item.unit_id} value={item.unit_id}>{item.public_name}</option>)}</select></Field>
      {!editing && <><Field label="Guest name"><input required value={form.guestName} onChange={event => change("guestName", event.target.value)} className="input" /></Field><Field label="Guest email"><input type="email" required value={form.guestEmail} onChange={event => change("guestEmail", event.target.value)} className="input" /></Field></>}
      <div className="grid grid-cols-2 gap-3"><Field label="Check-in"><input type="date" required value={form.checkIn} onChange={event => change("checkIn", event.target.value)} className="input" /></Field><Field label="Check-out"><input type="date" required value={form.checkOut} onChange={event => change("checkOut", event.target.value)} className="input" /></Field></div>
      <Field label="Guests"><input type="number" min="1" required value={form.guestCount} onChange={event => change("guestCount", event.target.value)} className="input" /></Field><Field label="Internal notes"><textarea value={form.ownerNotes} onChange={event => change("ownerNotes", event.target.value)} className="input" /></Field>
