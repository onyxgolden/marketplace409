...
export default function ReservationsPanel() {
  ...
  return <section aria-label="Reservations" className="space-y-5">
    ...
    <form onSubmit={event => { event.preventDefault(); submit(editing ? "preview_modification" : "preview"); }} className="space-y-3 rounded-2xl border bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <Field label="Space"><select required value={form.unitId} onChange={event => change("unitId", event.target.value)} className={inputClass}><option value="">Select a space</option>{inventory.map(item => <option key={item.unit_id} value={item.unit_id}>{item.public_name}</option>)}</select></Field>
