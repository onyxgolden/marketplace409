...
export default function RentalRecordBrowser({ title, records, selectedId, onSelect, getTitle, getSubtitle, getThumbnail, columns, children, emptyMessage }) {
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white lg:sticky lg:top-6 lg:flex lg:max-h-[calc(100vh-3rem)] lg:flex-col">
      ...
      {records.length === 0 ? <p className="p-4 text-sm text-slate-500">{emptyMessage}</p> : columns ? <div className="lg:overflow-y-auto">
        <table className="w-full table-fixed text-left text-sm">
          <thead className="sticky top-0 bg-slate-50"><tr>{getThumbnail && <th className="w-20 px-3 py-2" aria-hidden="true" />}{columns.map((column) => <th key={column.header} className="px-2 py-2 text-xs font-black uppercase tracking-wide text-slate-500 first:pl-4">{column.header}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-200">{records.map((record) => {
            ...
            return <tr key={record.id} aria-current={active ? "true" : undefined} role="button" tabIndex={0}
              ...
              {getThumbnail && <td className="w-20 px-3 py-3">{thumbnail
                ...
                : <span className="block h-14 w-14 min-h-14 min-w-14 rounded-lg bg-slate-100" aria-hidden="true" />}</td>}
      ...
      </div> : <div className="divide-y divide-slate-200 lg:overflow-y-auto">{records.map((record) => {
        const active = record.id === selectedId;
        const thumbnail = getThumbnail?.(record);
