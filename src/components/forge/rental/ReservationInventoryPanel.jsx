  ...
  return <section aria-label="RV and short-term rental inventory" className="space-y-5">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
      ...
      <form onSubmit={save} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5 text-slate-950 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100"><h3 className="font-black">Configure a space</h3>
        ...
        <div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Maximum nights <span className="font-normal">(blank = unlimited)</span><input type="number" min={form.minimumNights || 1} value={form.maximumNights} onChange={(e)=>update("maximumNights",e.target.value)} className="mt-1 w-full rounded-lg border p-2 dark:border-slate-600 dark:bg-slate-950" /></label><label className="text-sm font-bold">Booking status<select value={form.bookingStatus} onChange={(e)=>update("bookingStatus",e.target.value)} className="mt-1 w-full rounded-lg border p-2 text-slate-950 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100"><option value="draft">Draft</option><option value="active">Active</option><option value="paused">Paused</option><option value="inactive">Inactive</option></select></label></div>
