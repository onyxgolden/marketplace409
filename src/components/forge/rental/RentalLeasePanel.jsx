...
export default function RentalLeasePanel() {
  ...
  const [setup, setSetup] = useState({ units: [], tenants: [] });
  ...
  useEffect(() => { (async () => {
    ...
    setSetup({ units: result.units || [], tenants: result.tenants || [] });
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-rental-lease-setup>
    ...
    <p className="mt-2 text-sm text-slate-600">Select the saved unit and tenant. The schedule remains draft until the signed lease is ready.</p>
