...
export default function RentalPaymentsPanel({ initialData = null, initialAccount, dataScope = identity, initialShowSetup = false }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <RentalRecordBrowser title="Charges and payments" records={records} selectedId={activeId} onSelect={setSelectedId}
      ...
      getSubtitle={(item) => { if (item.kind !== "charge") return `${label(item.payment_method || item.provider)} · ${label(item.status)}`; const identity = resolveChargeIdentity(item, data); return `${label(item.charge_type)} · ${item.period} · ${money.format(Number(item.amount_cents) / 100)} · ${label(item.status)} · due ${item.due_date} · lease ${identity.leaseId || "Unknown"}`; }}
      emptyMessage="No rent charges or payments recorded.">
      {!selected ? <p className="text-sm text-slate-500">Select a charge or payment to review its details.</p> : <RecordDetail record={selected} chargeIdentity={selected.kind === "charge" ? resolveChargeIdentity(selected, data) : null} onRecordPayment={() => setShowOffline(true)} onVoidCharge={voidCharge} />}
