...
export default function RentalPaymentsPanel({ initialData = null, initialAccount, dataScope = identity, initialShowSetup = false, initialShowOffline = false, cacheKey = "rental:payments", initialViewFilter = null }) {
  ...
  const [selectedId, setSelectedId] = useState(""), [showOffline, setShowOffline] = useState(initialShowOffline), [showSetup, setShowSetup] = useState(initialShowSetup), [showBatch, setShowBatch] = useState(false);
  ...
  async function recordOffline(event) { event.preventDefault(); const element = event.currentTarget, form = new FormData(element); const success = await post({ operation: "record-offline-payment", payment: { chargeId: form.get("chargeId"), paymentMethod: form.get("paymentMethod"), amountCents: Math.round(Number(form.get("amount")) * 100), receivedAt: new Date(`${form.get("receivedDate")}T12:00:00`).toISOString(), receiptReference: form.get("receiptReference"), notes: form.get("notes"), depositState: form.get("depositState") === "deposited" ? "deposited" : "received" } }, () => "Offline payment recorded and rent balance updated."); if (success) { element.reset(); setShowOffline(false); } }
  ...
  return <section className="space-y-6" data-rental-payments>
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      {showOffline ? <form aria-label="Record offline payment" onSubmit={recordOffline} className="mt-6 grid gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/60 dark:bg-amber-950/30 md:grid-cols-2">
        ...
        <label className="text-sm font-bold text-slate-900 dark:text-white">Deposit status<select name="depositState" required defaultValue="received" className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal dark:border-slate-600 dark:bg-slate-900 dark:text-white"><option value="received">Received — awaiting deposit</option><option value="deposited">Already deposited</option></select></label>
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <RentalRecordBrowser title="Charges and payments" records={visibleRecords} selectedId={activeId} onSelect={setSelectedId}
        ...
        getSubtitle={(item) => { if (item.kind !== "charge") return `${label(item.payment_method || item.provider)} · ${label(item.status)} · ${depositStateLabel(item, item.settlement || null)}`; const identity = resolveChargeIdentity(item, data); return `${label(item.charge_type)} · ${item.period} · ${money.format(Number(item.amount_cents) / 100)} · ${label(item.status)} · due ${item.due_date} · lease ${identity.leaseId || "Unknown"} · ${chargeCollectionLabel(item, data.schedules)}`; }}
        emptyMessage="No rent charges or payments recorded.">
        {!selected ? <p className="text-sm text-slate-500 dark:text-slate-400">Select a charge or payment to review its details.</p> : <RecordDetail record={selected} chargeIdentity={selected.kind === "charge" ? resolveChargeIdentity(selected, data) : null} collectionLabel={selected.kind === "charge" ? chargeCollectionLabel(selected, data.schedules) : null} onRecordPayment={() => setShowOffline(true)} onVoidCharge={voidCharge} onSetDepositState={setDepositState} />}
      </RentalRecordBrowser>
...
function RecordDetail({ record, chargeIdentity, collectionLabel, onRecordPayment, onVoidCharge, onSetDepositState }) {
  ...
  const fundsState = resolveDepositState(record, record.settlement || null);
  const canTransitionFunds = MONEY_MOVED_STATUSES.has(record.status);
  return <div><p className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Selected payment</p><h3 className="mt-2 text-xl font-black text-slate-950 dark:text-white">{money.format(Number(record.amount_cents) / 100)} payment</h3><dl className="mt-5 grid gap-4 sm:grid-cols-2"><Fact term="Status" value={label(record.status)} /><Fact term="Method" value={label(record.payment_method || record.provider)} /><Fact term="Funds" value={depositStateLabel(record, record.settlement || null)} /><Fact term="Received" value={timestamp ? new Date(timestamp).toLocaleDateString() : "—"} /><Fact term="Refunded" value={money.format(Number(record.refunded_amount_cents || 0) / 100)} /><Fact term="Receipt" value={paymentReceiptReference(record)} /><Fact term="Settlement" value={record.provider === "stripe" ? label(record.settlement?.status || "awaiting settlement") : "Not applicable"} /></dl>
  {fundsState === DEPOSIT_STATE_RECEIVED ? <span className="mt-4 inline-block rounded-full bg-amber-100 px-3 py-1 text-xs font-black uppercase text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">Awaiting deposit</span> : <span className="mt-4 inline-block rounded-full bg-emerald-100 px-3 py-1 text-xs font-black uppercase text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">Deposited</span>}
  {canTransitionFunds ? (showDepositConfirm ? <div role="alertdialog" aria-label="Confirm deposit state change" className="mt-4 rounded-xl border border-slate-300 bg-white p-4 dark:border-slate-600 dark:bg-slate-900">
    <p className="text-sm font-bold text-slate-900 dark:text-white">{fundsState === DEPOSIT_STATE_RECEIVED ? "Mark this payment as deposited? The money is confirmed in the bank account." : "Mark this payment as awaiting deposit? Use this to correct a mistaken deposit mark."}</p>
    <div className="mt-3 flex items-center gap-3">
      <button type="button" onClick={async () => { const success = await onSetDepositState(record.sourceId, fundsState === DEPOSIT_STATE_RECEIVED ? DEPOSIT_STATE_DEPOSITED : DEPOSIT_STATE_RECEIVED); if (success) setShowDepositConfirm(false); }} className={`rounded-lg px-4 py-2 text-sm font-bold text-white transition ${goldControlClassName}`}>Confirm</button>
      <button type="button" onClick={() => setShowDepositConfirm(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</button>
    </div>
  </div> : <button type="button" onClick={() => setShowDepositConfirm(true)} className="mt-4 rounded-xl border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">{fundsState === DEPOSIT_STATE_RECEIVED ? "Mark as deposited" : "Mark as awaiting deposit"}</button>) : null}</div>;
}
