...
export default function TenantPaymentForm({ returnUrl, amountLabel, dueDate, onCancel }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    <div className="rounded-2xl bg-slate-950 p-5 text-white">
      <p className="text-sm text-slate-300">Rent payment</p><p className="mt-1 text-3xl font-black">{amountLabel}</p>
    ...
    <button disabled={!stripe || submitting} className="w-full rounded-xl bg-amber-500 px-5 py-3 font-black text-slate-950 disabled:opacity-50">
      {submitting ? "Submitting…" : "Submit rent payment"}
    </button>
