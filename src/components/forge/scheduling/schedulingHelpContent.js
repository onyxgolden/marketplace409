...
export const HELP_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Menu -> Export JSON / Import JSON", description: "Save the current project to a file, or load one back in." },
      { label: "Menu -> Export XER", description: "Downloads the project as a Primavera P6 .xer file -- activities, dependencies, calendars, and (if any) resources and costs." },
      { label: "Menu -> Export Project XML", description: "Downloads the project in Microsoft Project's XML interchange format -- opens directly in Project via File > Open." },
      { label: "Menu -> Export Excel / Import Excel", description: "Export downloads an editable Activities sheet (dates, duration, % complete, predecessors as CODE:TYPE+lag). Import updates existing activities by Task Code -- it can't create new ones, and every row is validated before anything changes, so a bad cell reports every error at once instead of applying half a file." },
      { label: "Menu -> Reset board", description: "Clears placed blocks, dependencies, and custom chips, and reverts lanes to this project's own template -- undoable if you didn't mean to. Keeps the project's name, dates, and calendar setup as they were." },
