...
export default function SchedulingInspector({ activeTab, onSelectTab, onCollapse, isOwner, ...panelProps }) {
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} type="button" role="tab" id={`scheduling-inspector-tab-${tab.id}`}
              ...
              className={`rounded-lg px-2.5 py-1.5 text-xs font-bold ${selected ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}>
              {tab.label}
            </button>
          );
        })}
