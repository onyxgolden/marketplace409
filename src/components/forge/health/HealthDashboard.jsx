...
function HealthDocumentImporter({ workspaceId, profiles, defaultCategory, onConfirmed }) {
  ...
  return <Card title="Upload a photo or document">
    ...
    {proposal?.proposal_type === "lab_results" && reviewed && <div className="space-y-3">
      ...
      {(reviewed.results || []).map((row, index) => <div key={`${row.markerName}-${index}`} className="grid gap-2 rounded-xl bg-slate-100 p-3 dark:bg-slate-800 sm:grid-cols-6"><input aria-label={`Marker ${index + 1}`} value={row.markerName || ""} onChange={(event) => updateResult(index, "markerName", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"/><input aria-label={`Value ${index + 1}`} value={row.valueNumeric ?? row.valueText ?? ""} onChange={(event) => updateResult(index, "valueNumeric", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"/><input aria-label={`Unit ${index + 1}`} value={row.unit || ""} onChange={(event) => updateResult(index, "unit", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"/><input aria-label={`Reference low ${index + 1}`} placeholder="Ref low" value={row.referenceLow ?? ""} onChange={(event) => updateResult(index, "referenceLow", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"/><input aria-label={`Reference high ${index + 1}`} placeholder="Ref high" value={row.referenceHigh ?? ""} onChange={(event) => updateResult(index, "referenceHigh", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"/><select aria-label={`Flag ${index + 1}`} value={row.flag || "unknown"} onChange={(event) => updateResult(index, "flag", event.target.value)} className="rounded-lg border bg-transparent px-2 py-1"><option className="text-slate-950" value="unknown">Unknown</option><option className="text-slate-950" value="normal">Normal</option><option className="text-slate-950" value="low">Low</option><option className="text-slate-950" value="high">High</option><option className="text-slate-950" value="critical">Critical</option></select></div>)}
    </div>}
