...
export default function FinancialAssetsPanel() {
  ...
  async function submit(event) {
    ...
    try {
      const response = await fetch("/api/financial/assets", {
        method: editingAssetId ? "PATCH" : "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ ...form, assetId: editingAssetId, valueCents: Math.round(Number(form.value) * 100), purchaseCostCents: form.purchaseCost === "" ? null : Math.round(Number(form.purchaseCost) * 100) }),
      });
      const body = await response.json();
  ...
  function beginEdit(asset) {
    ...
    setForm({
      ...
      value: Number(asset.latestValuation?.amountCents || 0) / 100,
      valueDate: today(),
      purchaseCost: asset.purchaseCostCents === null ? "" : Number(asset.purchaseCostCents) / 100,
      purchaseDate: asset.purchaseDate || "",
      linkedPropertyId: asset.linkedPropertyId || "",
      notes: asset.notes || "",
    });
    setShowForm(true);
  }
  ...
  async function retire(asset) {
    if (!window.confirm(`Retire ${asset.name}? Its history will be preserved, but it will leave active Net Worth.`)) return;
    setError("");
    const response = await fetch("/api/financial/assets", {
  ...
  return (
    <section data-financial-assets className="space-y-5">
      ...
      {showForm && <form onSubmit={submit} className="grid grid-cols-1 gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:grid-cols-2">
        <h3 className="md:col-span-2 text-xl font-black text-slate-950 dark:text-white">{editingAssetId ? "Update asset and record a new valuation" : "Add a physical or digital asset"}</h3>
        ...
        <label className="text-sm font-bold md:col-span-2">Linked rental property (optional)<select value={form.linkedPropertyId} onChange={(e) => setForm({ ...form, linkedPropertyId: e.target.value })} className="mt-1 w-full rounded-xl border px-3 py-2 dark:bg-slate-800"><option value="">Portfolio-wide / not property-specific</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.label}</option>)}</select></label>
        ...
        <div className="md:col-span-2 flex gap-3"><button disabled={saving} className={`rounded-full px-5 py-2 text-sm font-black ${goldControlClassName}`}>{saving ? "Saving…" : editingAssetId ? "Save new valuation" : "Save asset"}</button><button type="button" onClick={() => { setShowForm(false); setEditingAssetId(null); }} className="rounded-full bg-slate-100 px-5 py-2 text-sm font-black dark:bg-slate-800">Cancel</button></div>
      </form>}
