...
async function fetchInvestmentAccountsPayload() {
...
export default function InvestmentAccountsPanel() {
  ...
  const { data, error: loadError, isLoading, isRefreshing, refresh } = useStaleWhileRevalidate(
    ...
    fetchInvestmentAccountsPayload,
    { ttlMs: 60_000 },
  );
  ...
  async function submit(event) {
    ...
    try {
      ...
      setForm(emptyForm()); setEditingAccountId(null); setShowForm(false); await refresh();
  ...
  return (
    <section data-investment-accounts className="space-y-5">
      {!accounts && isLoading ? <ForgeLoadingState label="Loading investment accounts…" /> : null}
      {!accounts && !isLoading && loadError ? (
        <ForgeErrorState
          title="Could not load your investment accounts."
          detail={loadError}
          onRetry={refresh}
        />
      ) : null}
      ...
      <div className="rounded-3xl border border-slate-700 bg-[linear-gradient(135deg,#111b31_0%,#0b1325_55%,#17233c_100%)] p-6 text-white shadow-xl">
        ...
        {loadError ? <p role="status" className="mt-4 text-xs font-bold text-slate-400">Could not refresh — showing the last saved accounts.</p> : null}
