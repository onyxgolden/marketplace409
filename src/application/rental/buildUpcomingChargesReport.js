...
export function buildUpcomingChargesReport({units=[],tenants=[],leases=[],memberships=[],charges=[]},today=new Date().toISOString().slice(0,10),windowDays=30){
  ...
  const rows=charges.filter(charge=>["scheduled","due","partially_paid"].includes(charge.status)&&charge.due_date>=today&&charge.due_date<=cutoff).map(charge=>{
  ...
  return Object.freeze({generatedAt:new Date().toISOString(),asOfDate:today,windowDays,
