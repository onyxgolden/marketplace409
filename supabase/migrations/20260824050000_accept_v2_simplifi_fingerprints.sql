-- fingerprintSimplifiRows.js is being corrected in the same change (a real cardinality bug: it
-- treated Simplifi's split_identity flag, "yes"/"no", as if it were a unique per-row identity,
-- collapsing N genuinely distinct otherwise-identical transactions onto one v1 fingerprint and
-- silently dropping N-1 of them). New fingerprints are minted as "v2:<hex>" going forward; existing
-- "v1:<hex>" source_record_id values already stored in financial_events are untouched and must
-- keep validating too, since recovery reuses them verbatim for rows that match an already-imported
-- legacy transaction (they're classified already_imported and never resubmitted, but accepting only
-- "v1:" here would be a latent trap the moment version numbers move again). Every other check in the
-- function is unchanged from 20260824040000; only the fingerprint format and the previously
-- hardcoded 'v1' fingerprint_version (now taken from the row, which is always 'v2' for anything
-- actually reaching this far, since already_imported rows never get submitted) are updated.
