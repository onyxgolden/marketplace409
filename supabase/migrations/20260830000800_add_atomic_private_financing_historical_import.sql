...
create table if not exists public.private_financing_import_batches (
    ...
    plan_digest text not null check (plan_digest ~ '^[0-9a-f]{64} check (payment_event_count >= 0),
...
begin
    ...
    insert into public.private_financing_import_batches (
        owner_id, source_key, account_id, payment_event_count, credit_event_count, created_by
    ) values (
        p_owner_id, p_source_key, v_account.id, v_payment_count, v_credit_count,
        v_authenticated_user::text
    );
