-- Rentec financial-history resume import: safely fills the gap between what Rentec's own
-- transaction ledger has always had and what FORGE's financial_events table actually holds (the
-- historical CSV-based import stopped partway through 2020; this resumes it from the live Rentec
-- API). This is financial history only — it never creates rental_payments, rent_charges,
-- settlements, or reconciliation approvals, and never touches those tables.
--
-- The real idempotency guarantee is financial_events' own existing unique index
-- (owner_id, source_system, source_record_id) from 20260714_create_financial_events.sql — this
-- migration does not need a side dedupe table the way rentec_transaction_imports needed one, because
-- financial_events already has a natural conflict target to insert against. What this migration adds
-- is (1) an owner-scoped audit summary of each import batch, and (2) the approval function itself,
-- which trusts nothing about the financial facts from its caller except which pre-classified rows to
-- apply — every row's shape and semantics are revalidated here before anything is written.
--
-- NOT applied remotely by this change — local migration file only.

create table if not exists rentec_financial_history_import_batches (
    id text primary key default ('rentec_fin_hist_batch_' || gen_random_uuid()::text),
...
create index if not exists idx_rentec_financial_history_import_batches_owner
  on rentec_financial_history_import_batches (owner_id, approved_at desc);

alter table rentec_financial_history_import_batches enable row level security;
alter table rentec_financial_history_import_batches force row level security;
create policy "rentec_financial_history_import_batches_owner_select" on rentec_financial_history_import_batches
  for select to authenticated using (owner_id = auth.uid()::text);
-- Without this, approve_rentec_financial_history_import() — SECURITY INVOKER, so its writes run as
-- the calling 'authenticated' role — would have no policy permitting its own audit-summary INSERT.
create policy "rentec_financial_history_import_batches_owner_insert" on rentec_financial_history_import_batches
  for insert to authenticated with check (owner_id = auth.uid()::text);

-- Owner-authenticated, owner-scoped, idempotent bulk import of previewed "safe missing" Rentec
-- financial-history rows. p_rows is a jsonb array; every row is fully revalidated here (required
-- fields, transaction_kind/enum membership, positive amount) rather than trusted from the caller —
-- the caller (the approval API route) is responsible for having freshly re-fetched Rentec and
-- re-run the same preview classifier immediately before calling this, so nothing here should ever
-- actually fail that check in normal operation, but the check exists so a client cannot forge an
-- arbitrary financial_events row through this path. source_system is always forced to 'rentec_api'
-- here, regardless of what (if anything) the caller's row includes — never trusted from the client.
-- A row whose (owner_id, source_system, source_record_id) already exists is silently skipped
-- (on conflict do nothing) rather than erroring, so re-running an approval batch — including two
-- concurrent approvals of overlapping rows — can never insert a duplicate financial_events row.
...
begin
    ...
    select x.reason into v_invalid_reason
